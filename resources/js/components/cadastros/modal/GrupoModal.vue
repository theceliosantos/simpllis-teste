...
async function criarOuSelecionar() {
  ...
  const existente = props.grupos.find(
  ...
  try {
    const { data } = await axios.post('/api/grupos', { nome })

    emit('atualizado', data)
    selecionar(data)

  } catch (err) {
    mensagemErro.value = 'Erro ao criar grupo.'
  } finally {
    criando.value = false
  }
}
